...
Last updated: end of the Documents phase. Phases D, E1, E2, F, and Documents are all committed and pushed. Phase E2 needed one follow-up fix after the first natural overnight cron run surfaced a real bug — see "Phase E2" below. Phase F surfaced three more real bugs during QA — see "Phase F" below. Documents reused Phase F's Storage infrastructure directly and passed its full QA pass clean on the first run — see "Documents" below.

**Note on parallel sessions:** a separate Claude Code session did an unrelated UI refactor (`f2ae9b9`, moving Weight/Reminder "+Add" triggers into header rows) concurrently with Phase F's session, and started `HAIRBALL_LOG.md` (a bug-tracking companion doc at the project root, now committed by Zee). Both are now merged in. Worth remembering this can happen again: check `git status` at the start of a session before assuming you know everything in the working tree.
...
**Not done yet:** Zee's own manual browser walkthrough (including a real phone photo upload — HEIC previews reliably only in Safari, flagged as an acceptable MVP caveat, not blocking).

---

## Documents

**What:** SPEC reqs 25-27. Per the plan at `~/.claude/plans/linked-launching-origami.md`: upload a file (PDF/JPG/PNG, 10MB default, server-validated), view/download/delete, and an optional link to one of that pet's existing care events or symptom entries — confirmed with Zee as a real dropdown pick, not free text (SPEC left this technically undefined; the mockup only ever showed static illustrative labels with no picker UI). Lands as a third Actions accordion card, after Symptoms and Vet report.

- `web/supabase/migrations/0007_documents.sql` — `documents` table with two nullable FKs (`care_event_id`, `symptom_entry_id`, both `on delete set null`) instead of an unenforced polymorphic `(type, id)` pair, capped at one via `check (num_nonnulls(...) <= 1)` — deleting a linked care event or symptom entry leaves the document intact, just unlinked, verified directly. Same owner-via-`pets`-join RLS pattern as everywhere else, plus a bucket-widening `update` adding `application/pdf` to `pet-attachments`' `allowed_mime_types`.
- **Reused Phase F's Storage infrastructure directly and it just worked** — `lib/storage/attachments.ts`'s `uploadAttachment`/`removeAttachments`/`getSignedAttachmentUrls`/`getAuthedStorage` needed zero changes; only additions were `ALLOWED_DOCUMENT_MIME_TYPES` and a new `getSignedAttachmentDownloadUrl` (a document's forced-download `Content-Disposition` needs its *original* filename, not the UUID-based storage path — `getSignedAttachmentUrls`' batched call can't vary that per path, so Download uses a separate per-document signed URL via `createSignedUrl`'s `download` option).
- `web/src/lib/documents/{types,queries,actions,format}.ts` — mirrors `lib/symptoms/`'s shape. `uploadDocument` follows the same ownership-check → validate → upload → insert → cleanup-on-failure ordering as `createSymptomEntry`, plus an extra check that closes a real gap the schema leaves unenforced: confirming a chosen link (care event or symptom entry) actually belongs to *this* pet, not just that it exists somewhere.
- **First real delete feature in the app** — every prior record type (weight, care events, symptoms) deliberately shipped with no delete UI. `deleteDocument` deletes the DB row first, then best-effort removes the Storage object (the reverse of upload's order — a DB-delete failure after a successful Storage removal would leave a *visibly listed* row with a broken View/Download link, worse than an invisible orphaned blob). UI is a plain `window.confirm()`, not a custom confirm component — deliberate, low-cost call for a first, low-blast-radius delete affordance, not a precedent meant to bind future riskier deletes. Note: `deleteDocument` returns `Promise<void>` and throws on error (matching `reminders/actions.ts`'s `markReminderDone`/`cancelReminder`), *not* the `{error}/{success}` `FormState` shape every upload-style action uses — a plain `<form action={fn}>` without `useActionState` needs that exact signature, caught by `tsc`.
- Upload UI (`DocumentUploadZone.tsx`) is real native HTML5 drag-and-drop built from scratch — the mockup's `.upload-drop` zone had zero JS behavior, purely decorative. One persistent (always-mounted, visually hidden until needed) `<input type="file">` serves both the click-to-pick and drop paths, so a dropped file's `FileList` survives into the same form's submission. Selecting a file swaps the drop zone for an inline confirmation card (not a modal — the drop zone is already the focal surface, popping a `<dialog>` over it would add a disconnected layer for no benefit) with the optional link `<select>`.
- Hit a genuinely new React 19 lint rule (`react-hooks/set-state-in-effect`) flagging the "reset local state after a Server Action succeeds" pattern used elsewhere in this app (calling `setState` inside a `useEffect` watching `useActionState`'s result). Fixed using React's own documented "adjusting state during render" pattern (comparing against a state-tracked previous value, calling `setState` directly in the render body) instead of an effect — worth applying the same fix if this rule ever flags `SymptomForm`/`ReminderForm`/`WeightForm`'s similar effects later.

**Verified as actually working:** `tsc --noEmit`, `pnpm lint`, `pnpm test` (83 tests) clean throughout. Migration applied live and confirmed via direct SQL (table, check constraint, RLS policies, widened bucket MIME list all present). A scripted Playwright pass against two throwaway accounts covered: upload with no link / linked to a care event / linked to a symptom entry, every allowed MIME type including PDF, disallowed-type and oversized-file rejection, the exactly-10MB inclusive boundary, a bypassed cross-pet link rejection (submitting a link value the UI would never offer), View and Download signed URLs (including a direct `Content-Disposition` filename assertion), delete removing both the DB row and the Storage object (confirmed via direct Storage API listing, not UI trust), the at-most-one-link constraint, the on-delete-set-null cascade behavior, cross-account RLS isolation for both the new table and the reused bucket, and the Storage bucket itself independently rejecting a disallowed MIME type. **All 22 scenarios passed on the first full run** — strong confirmation that Phase F's Storage groundwork was solid. Test accounts, data, Storage objects, and the temporary `playwright` devDependency all removed afterward.

**Not done yet:** Zee's own manual walkthrough (a real PDF and a real phone photo, plus trying actual drag-and-drop in a real browser — only Playwright's synthetic `setInputFiles`/dispatched drop event was exercised, not a literal OS-level drag gesture).

---

## Next up

1. Decide what to do with the stray uncommitted `SPEC.md` PWA section (still unrelated, still sitting in the working tree since Phase E2).
2. Whatever's next per SPEC's build order after documents: **Care Card** (SPEC reqs 28-30 — read-only emergency summary, export/print as PDF, explicitly no public/unauthenticated link in MVP). The app already has a Care Card placeholder page and topbar link from early scaffolding (Phase 1/D) — worth checking its current state before planning, it may already be closer to done than a fresh read of SPEC would suggest.
